...
module.exports = function (app, passport) {
    ...
    // 사용자 인증 성공 시 호출
    // 사용자 정부를 이용해 세션을 만듦
    // 로그인 이후에 들어오는 요청은 deserializeUser 메소드 안에서 해당 세션을 확인할 수 있음
    ...
    // user -> 사용자 인증 성공 시 serializeUser 메소드를 이용해 만들었던 세션 정보가 파라미터로 넘어온 것
    passport.deserializeUser(function (user, done) {
        console.log('deserializeUser() 호출됨.');
        // console.dir(user);

        // 사용자 정보 중 id나 email만 있는 경우 사용자 정보 조회 필요 - 여기에서는 user 객체 전체를 패스포트과 관리
